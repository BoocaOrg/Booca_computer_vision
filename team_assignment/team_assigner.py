...
class TeamAssigner:
    ...
    def assign_team_colour(self, frame: np.ndarray, tracks: Dict[str, List[Dict]], force: bool = False) -> None:
        ...
        # Need at least 2 players to determine 2 teams
        if len(player_colours) < 2:
            # If only 1 player or less, assign a default team color
            if len(player_colours) == 1:
                self.team_colours[1] = player_colours[0]
                self.team_colours[2] = player_colours[0]  # Same color for both teams
                # Create a simple kmeans-like object for consistency
                self.kmeans = type('obj', (object,), {
                    'predict': lambda self, x: np.array([0])
                })()
            return
    ...
    def get_player_team(self, frame: np.ndarray, bbox: List[float], player_id: int) -> int:
        ...
        # Safety check: if kmeans not initialized, default to team 1
